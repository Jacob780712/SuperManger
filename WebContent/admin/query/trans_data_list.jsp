...
<html>
		        ...
		        for (i = rowBeginIndex; i < tabObj.rows.length; i++) {
		            ...
		            for (j = i + 1; j < tabObj.rows.length; j++) {
		                if (strTemp == tabObj.rows[j].cells[colIndex].innerHTML) {
		                    ...
		                    tabObj.rows[i].cells[7].innerHTML = parseInt(tabObj.rows[i].cells[7].innerHTML) +
                    			parseInt(tabObj.rows[j].cells[7].innerHTML);
	...
	<body>
		<form name="frmPage" id="queryFrom" action="transData.jhtml" method="post">
			<div>
				<div align="left">
					您现在的位置：查询统计 &gt;&gt;交易统计
				</div>
				...
				<table class="table_a marginb10" width="90%">
					<tr>
						<td align="center">交易总人数
						</td>
						<td align="center">两次交易及以上总人数
						</td>
						<td align="center">交易总笔数
						</td>
						<td align="center">交易总金额
						</td>
						<td align="center">储值卡支付金额
						</td>
						<td align="center">账户支付总金额
						</td>
						<td align="center">微信支付总金额
						</td>
						<td align="center">平台返现总金额
						</td>
						<td align="center">储值用户返现总金额
						</td>
					</tr>
					<tr>
						<td align="center">
							${TransDataBo.allrenshu}
						</td>
						<td align="center">
							${TransDataBo.twomd}
						</td>
						<td align="center">
							${TransDataBo.allBishu}
						</td>
						<td align="center">
							<fmt:formatNumber value="${TransDataBo.all_bussiness_amount/100}" pattern="##0.00"/>
						</td>
						<td align="center">
							<fmt:formatNumber value="${TransDataBo.svc_pay_amount/100}" pattern="##0.00"/>
						</td>
						<td align="center">
							<fmt:formatNumber value="${TransDataBo.account_pay_amount/100}" pattern="##0.00"/>
						</td>
						<td align="center">
							<fmt:formatNumber value="${TransDataBo.all_pay_amount/100}" pattern="##0.00"/>
						</td>
						<td align="center">
							<fmt:formatNumber value="${TransDataBo.cofanxian/100}" pattern="##0.00"/>
						</td>
						<td align="center">
							<fmt:formatNumber value="${TransDataBo.fanxian/100}" pattern="##0.00"/>
						</td>
					</tr>
				</table>
				<table class="table_a marginb10" id="tab" width="90%">
					...
					<c:forEach var="item" items="${page.result}" varStatus="status">
						<tr>
							...
							<td align="center">
								<fmt:formatNumber value="${item.bussiness_amount/100}" pattern="##0.00"/>
							</td>
							<td align="center">
							...
							<td align="center">
								<fmt:formatNumber value="${item.fanxian/100}" pattern="##0.00"/>
							</td>
							<td align="center">
								<fmt:formatNumber value="${item.cofanxian/100}" pattern="##0.00"/>
							</td>
